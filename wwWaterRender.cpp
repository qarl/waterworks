...
void wwWaterRender::build_height_table()
{
  int i;
...
void wwWaterRender::build_thickness_table()
{
  int i;
  for (i = 0; i < HEIGHT_RES; i++)
    {
      thickness[i] = 0.9 + 
	0.1 * (pow((float)(HEIGHT_RES - i) / (float)HEIGHT_RES ,1.1));
    }

}

void wwWaterRender::build_fresnel_table()
{
  int i;
  float slope;
  ...
  for (i = 0; i < 2*HEIGHT_RES; i++)
    {
      ...
      fresnel[i] = 1.0 - pow(fabs(slope), 0.9) + 0.0;
    }
}



void wwWaterRender::build_specular_table()
{
  int i;
  float slope;
...
void wwWaterRender::build_refract_table()
{
  int i;
  float slope;
  ...
  for (i = 0; i < 2 * HEIGHT_RES; i++)
    {
      slope = ((float)i - HEIGHT_RES) / (float)HEIGHT_RES;
      refract[i] = (int)(-slope * 100);
    }
}


void wwWaterRender::build_color_table()
{
  int i;
...
void wwWaterRender::make_height_blur(const wwMapValue &water)
{
  int x, y, a;
...
void wwWaterRender::draw(const wwMapColor &image, const wwMapValue &water,
			 wwMapColor &output)
{
  int x, y, c;
	...
	if (h)
	  {
	    ...
	    output.colors[y][x][0] =
	      clip256((int)(image.colors[ry][rx][0] *
	    ...
	    output.colors[y][x][1] =
	      clip256((int)(image.colors[ry][rx][1] *
			    ...
			    + 256 * specular[xslope]*specular[yslope]));
	    output.colors[y][x][2] =
	      clip256((int)(image.colors[ry][rx][2] *
			    fresnel[xslope] * fresnel[yslope] *
			    thickness[h]
			    + 256 * specular[xslope]*specular[yslope]));
	  }
...
void wwWaterRender::doIt(const wwMapColor &image, const wwMapValue &water,
			 wwMapColor &output)
{
